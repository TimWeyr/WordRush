...
export class BaseEntity extends GameObject {
  ...
  private renderCrosshair(renderer: Renderer, color: string): void {
    ...
    // Crosshair lines
    const lineLength = radius * 0.4;
    // Horizontal
    ctx.beginPath();
    ctx.moveTo(this.position.x - lineLength, this.position.y);
    ctx.lineTo(this.position.x + lineLength, this.position.y);
    ctx.stroke();
    // Vertical
    ctx.beginPath();
    ctx.moveTo(this.position.x, this.position.y - lineLength);
    ctx.lineTo(this.position.x, this.position.y + lineLength);
    ctx.stroke();
    
    // Corner brackets
    const bracketSize = 15;
    const bracketOffset = radius * 0.8;
    ctx.lineWidth = 3;
    for (let i = 0; i < 4; i++) {
      const angle = (Math.PI / 2) * i;
      const cx = this.position.x + Math.cos(angle) * bracketOffset;
      const cy = this.position.y + Math.sin(angle) * bracketOffset;
      
      ctx.beginPath();
      ctx.moveTo(cx - bracketSize * Math.sin(angle), cy + bracketSize * Math.cos(angle));
      ctx.lineTo(cx, cy);
      ctx.lineTo(cx + bracketSize * Math.sin(angle), cy - bracketSize * Math.cos(angle));
      ctx.stroke();
    }
